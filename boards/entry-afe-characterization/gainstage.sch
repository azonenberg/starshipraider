EESchema Schematic File Version 4
LIBS:entry-afe-characterization-cache
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 6 8
Title "Entry-Level Oscilloscope AFE Characterization Platform"
Date "2020-03-29"
Rev "0.1"
Comp "Antikernel Labs"
Comment1 ""
Comment2 "Andrew D. Zonenberg"
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L analog-azonenberg:ADL5205 U?
U 2 1 5E744B89
P 2800 2700
AR Path="/5E744B89" Ref="U?"  Part="2" 
AR Path="/5E743C43/5E744B89" Ref="U2"  Part="2" 
F 0 "U2" H 3325 3325 50  0000 C CNN
F 1 "ADL5205" H 3325 3234 50  0000 C CNN
F 2 "azonenberg_pcb:QFN_40_0.5MM_6x6MM" H 2800 2750 50  0001 C CNN
F 3 "" H 2800 2750 50  0001 C CNN
	2    2800 2700
	1    0    0    -1  
$EndComp
$Comp
L analog-azonenberg:ADL5205 U?
U 3 1 5E744B8F
P 2800 1500
AR Path="/5E744B8F" Ref="U?"  Part="3" 
AR Path="/5E743C43/5E744B8F" Ref="U2"  Part="3" 
F 0 "U2" H 3325 2125 50  0000 C CNN
F 1 "ADL5205" H 3325 2034 50  0000 C CNN
F 2 "azonenberg_pcb:QFN_40_0.5MM_6x6MM" H 2800 1550 50  0001 C CNN
F 3 "" H 2800 1550 50  0001 C CNN
	3    2800 1500
	1    0    0    -1  
$EndComp
$Comp
L analog-azonenberg:ADL5205 U?
U 1 1 5E748048
P 2800 5600
AR Path="/5E748048" Ref="U?"  Part="1" 
AR Path="/5E743C43/5E748048" Ref="U2"  Part="1" 
F 0 "U2" H 3375 7625 50  0000 C CNN
F 1 "ADL5205" H 3375 7534 50  0000 C CNN
F 2 "azonenberg_pcb:QFN_40_0.5MM_6x6MM" H 2800 5650 50  0001 C CNN
F 3 "" H 2800 5650 50  0001 C CNN
	1    2800 5600
	1    0    0    -1  
$EndComp
Text HLabel 2700 1100 0    50   Input ~ 0
GAIN_PWRUP
Text HLabel 2700 1400 0    50   Input ~ 0
VSHIFTED_P
Text HLabel 2700 1300 0    50   Input ~ 0
VSHIFTED_N
Text HLabel 3950 1400 2    50   Output ~ 0
VGAIN_P
Text HLabel 3950 1300 2    50   Output ~ 0
VGAIN_N
Text Notes 2800 2800 0    50   ~ 0
Break unused amp channel out to SMAs
$Comp
L Connector:Conn_Coaxial J3
U 1 1 5E74ED29
P 1550 2650
F 0 "J3" H 1400 2600 50  0000 C CNN
F 1 "SMA" H 1400 2500 50  0000 C CNN
F 2 "azonenberg_pcb:CONN_SMA_EDGE_SAMTEC_SMA_J_P_H_ST_EM1" H 1550 2650 50  0001 C CNN
F 3 "" H 1550 2650 50  0001 C CNN
	1    1550 2650
	-1   0    0    -1  
$EndComp
$Comp
L Connector:Conn_Coaxial J2
U 1 1 5E750662
P 1550 2450
F 0 "J2" H 1400 2400 50  0000 C CNN
F 1 "SMA" H 1400 2300 50  0000 C CNN
F 2 "azonenberg_pcb:CONN_SMA_EDGE_SAMTEC_SMA_J_P_H_ST_EM1" H 1550 2450 50  0001 C CNN
F 3 "" H 1550 2450 50  0001 C CNN
	1    1550 2450
	-1   0    0    1   
$EndComp
Wire Wire Line
	1350 2850 1550 2850
Text Label 1350 2850 2    50   ~ 0
GND
Text Label 2200 2500 0    50   ~ 0
GIN_P
Wire Wire Line
	1700 2450 1800 2450
Text Label 2200 2600 0    50   ~ 0
GIN_N
Text Label 3950 2300 0    50   ~ 0
2V5_REF
$Comp
L Connector:Conn_Coaxial J4
U 1 1 5E753967
P 4750 2450
F 0 "J4" H 4600 2400 50  0000 C CNN
F 1 "SMA" H 4600 2300 50  0000 C CNN
F 2 "azonenberg_pcb:CONN_SMA_EDGE_SAMTEC_SMA_J_P_H_ST_EM1" H 4750 2450 50  0001 C CNN
F 3 "" H 4750 2450 50  0001 C CNN
	1    4750 2450
	1    0    0    1   
$EndComp
Wire Wire Line
	1800 2450 1800 2500
Wire Wire Line
	1800 2500 2700 2500
Wire Wire Line
	1700 2650 1800 2650
Wire Wire Line
	1800 2650 1800 2600
Wire Wire Line
	1800 2600 2700 2600
Wire Wire Line
	1350 2850 1350 2250
Wire Wire Line
	1350 2250 1550 2250
$Comp
L Connector:Conn_Coaxial J5
U 1 1 5E7562B4
P 4750 2650
F 0 "J5" H 4600 2600 50  0000 C CNN
F 1 "SMA" H 4600 2500 50  0000 C CNN
F 2 "azonenberg_pcb:CONN_SMA_EDGE_SAMTEC_SMA_J_P_H_ST_EM1" H 4750 2650 50  0001 C CNN
F 3 "" H 4750 2650 50  0001 C CNN
	1    4750 2650
	1    0    0    -1  
$EndComp
Wire Wire Line
	4600 2650 4500 2650
Wire Wire Line
	4500 2650 4500 2600
Wire Wire Line
	4500 2600 3950 2600
Wire Wire Line
	3950 2500 4500 2500
Wire Wire Line
	4500 2500 4500 2450
Wire Wire Line
	4500 2450 4600 2450
Wire Wire Line
	5050 2850 4750 2850
Text Label 5050 2850 0    50   ~ 0
GND
Text Label 4050 2500 0    50   ~ 0
GOUT_P
Text Label 4050 2600 0    50   ~ 0
GOUT_N
Text HLabel 3950 1100 2    50   Input ~ 0
2V5_REF
Text HLabel 2700 2300 0    50   Input ~ 0
GAIN2_PWRUP
Text HLabel 2550 3800 0    50   Input ~ 0
5V0_P
Wire Wire Line
	2550 3800 2700 3800
Wire Wire Line
	2700 3800 2700 3900
Connection ~ 2700 3800
Text HLabel 2550 4100 0    50   Input ~ 0
GND
Wire Wire Line
	2550 4100 2700 4100
$Comp
L device:R R16
U 1 1 5E77B9A7
P 2150 4300
F 0 "R16" V 2100 4150 50  0000 C CNN
F 1 "10K" V 2150 4300 50  0000 C CNN
F 2 "azonenberg_pcb:EIA_0402_RES_NOSILK" V 2080 4300 50  0001 C CNN
F 3 "" H 2150 4300 50  0001 C CNN
	1    2150 4300
	0    1    1    0   
$EndComp
$Comp
L device:R R17
U 1 1 5E77C06E
P 2150 4400
F 0 "R17" V 2100 4250 50  0000 C CNN
F 1 "10K" V 2150 4400 50  0000 C CNN
F 2 "azonenberg_pcb:EIA_0402_RES_NOSILK" V 2080 4400 50  0001 C CNN
F 3 "" H 2150 4400 50  0001 C CNN
	1    2150 4400
	0    1    1    0   
$EndComp
Wire Wire Line
	2300 4400 2700 4400
Wire Wire Line
	2700 4300 2300 4300
Text Label 1800 4300 2    50   ~ 0
5V0_P
Wire Wire Line
	1800 4300 1900 4300
Wire Wire Line
	1900 4300 1900 4400
Wire Wire Line
	1900 4400 2000 4400
Connection ~ 1900 4300
Wire Wire Line
	1900 4300 2000 4300
$Comp
L device:R R18
U 1 1 5E77DCCA
P 2150 4600
F 0 "R18" V 2100 4450 50  0000 C CNN
F 1 "10K" V 2150 4600 50  0000 C CNN
F 2 "azonenberg_pcb:EIA_0402_RES_NOSILK" V 2080 4600 50  0001 C CNN
F 3 "" H 2150 4600 50  0001 C CNN
	1    2150 4600
	0    1    1    0   
$EndComp
$Comp
L device:R R19
U 1 1 5E77E234
P 2150 4700
F 0 "R19" V 2100 4550 50  0000 C CNN
F 1 "10K" V 2150 4700 50  0000 C CNN
F 2 "azonenberg_pcb:EIA_0402_RES_NOSILK" V 2080 4700 50  0001 C CNN
F 3 "" H 2150 4700 50  0001 C CNN
	1    2150 4700
	0    1    1    0   
$EndComp
Wire Wire Line
	2300 4600 2700 4600
Wire Wire Line
	2700 4700 2300 4700
Text Label 1800 4600 2    50   ~ 0
GND
Wire Wire Line
	1800 4600 1900 4600
Wire Wire Line
	1900 4600 1900 4700
Wire Wire Line
	1900 4700 2000 4700
Connection ~ 1900 4600
Wire Wire Line
	1900 4600 2000 4600
$Comp
L device:R R20
U 1 1 5E780489
P 2150 4900
F 0 "R20" V 2100 4750 50  0000 C CNN
F 1 "10K" V 2150 4900 50  0000 C CNN
F 2 "azonenberg_pcb:EIA_0402_RES_NOSILK" V 2080 4900 50  0001 C CNN
F 3 "" H 2150 4900 50  0001 C CNN
	1    2150 4900
	0    1    1    0   
$EndComp
$Comp
L device:R R21
U 1 1 5E7809A3
P 2150 5000
F 0 "R21" V 2100 4850 50  0000 C CNN
F 1 "10K" V 2150 5000 50  0000 C CNN
F 2 "azonenberg_pcb:EIA_0402_RES_NOSILK" V 2080 5000 50  0001 C CNN
F 3 "" H 2150 5000 50  0001 C CNN
	1    2150 5000
	0    1    1    0   
$EndComp
Text Label 1800 4900 2    50   ~ 0
GND
Wire Wire Line
	1800 4900 1900 4900
Wire Wire Line
	1900 4900 1900 5000
Wire Wire Line
	1900 5000 2000 5000
Connection ~ 1900 4900
Wire Wire Line
	1900 4900 2000 4900
Wire Wire Line
	2300 4900 2700 4900
Wire Wire Line
	2300 5000 2700 5000
$Comp
L device:R R22
U 1 1 5E7834EF
P 2150 5250
F 0 "R22" V 2100 5100 50  0000 C CNN
F 1 "10K" V 2150 5250 50  0000 C CNN
F 2 "azonenberg_pcb:EIA_0402_RES_NOSILK" V 2080 5250 50  0001 C CNN
F 3 "" H 2150 5250 50  0001 C CNN
	1    2150 5250
	0    1    1    0   
$EndComp
$Comp
L device:R R23
U 1 1 5E783837
P 2150 5350
F 0 "R23" V 2100 5200 50  0000 C CNN
F 1 "10K" V 2150 5350 50  0000 C CNN
F 2 "azonenberg_pcb:EIA_0402_RES_NOSILK" V 2080 5350 50  0001 C CNN
F 3 "" H 2150 5350 50  0001 C CNN
	1    2150 5350
	0    1    1    0   
$EndComp
$Comp
L device:R R24
U 1 1 5E783DAE
P 2150 5450
F 0 "R24" V 2100 5300 50  0000 C CNN
F 1 "10K" V 2150 5450 50  0000 C CNN
F 2 "azonenberg_pcb:EIA_0402_RES_NOSILK" V 2080 5450 50  0001 C CNN
F 3 "" H 2150 5450 50  0001 C CNN
	1    2150 5450
	0    1    1    0   
$EndComp
$Comp
L device:R R25
U 1 1 5E783FC7
P 2150 5550
F 0 "R25" V 2100 5400 50  0000 C CNN
F 1 "10K" V 2150 5550 50  0000 C CNN
F 2 "azonenberg_pcb:EIA_0402_RES_NOSILK" V 2080 5550 50  0001 C CNN
F 3 "" H 2150 5550 50  0001 C CNN
	1    2150 5550
	0    1    1    0   
$EndComp
Wire Wire Line
	2300 5250 2700 5250
Wire Wire Line
	2700 5350 2300 5350
Wire Wire Line
	2300 5450 2700 5450
Wire Wire Line
	2700 5550 2300 5550
Text Label 1800 5250 2    50   ~ 0
GND
Wire Wire Line
	1800 5250 1900 5250
Wire Wire Line
	1900 5250 1900 5350
Wire Wire Line
	1900 5350 2000 5350
Connection ~ 1900 5250
Wire Wire Line
	1900 5250 2000 5250
Wire Wire Line
	1900 5350 1900 5450
Wire Wire Line
	1900 5450 2000 5450
Connection ~ 1900 5350
Wire Wire Line
	1900 5450 1900 5550
Wire Wire Line
	1900 5550 2000 5550
Connection ~ 1900 5450
$Comp
L device:R R27
U 1 1 5E790B70
P 4400 3900
F 0 "R27" V 4350 3750 50  0000 C CNN
F 1 "10K" V 4400 3900 50  0000 C CNN
F 2 "azonenberg_pcb:EIA_0402_RES_NOSILK" V 4330 3900 50  0001 C CNN
F 3 "" H 4400 3900 50  0001 C CNN
	1    4400 3900
	0    1    1    0   
$EndComp
$Comp
L device:R R26
U 1 1 5E791612
P 4400 3800
F 0 "R26" V 4350 3650 50  0000 C CNN
F 1 "10K" V 4400 3800 50  0000 C CNN
F 2 "azonenberg_pcb:EIA_0402_RES_NOSILK" V 4330 3800 50  0001 C CNN
F 3 "" H 4400 3800 50  0001 C CNN
	1    4400 3800
	0    1    1    0   
$EndComp
Wire Wire Line
	4250 3800 4050 3800
Wire Wire Line
	4050 3900 4250 3900
Text Label 4700 3900 0    50   ~ 0
GND
Wire Wire Line
	4700 3900 4550 3900
Text Label 4700 3800 0    50   ~ 0
5V0_P
Wire Wire Line
	4700 3800 4550 3800
Text HLabel 4300 4200 2    50   Input ~ 0
SPI_CS_A_N
Wire Wire Line
	4300 4200 4050 4200
Text HLabel 4300 4300 2    50   Input ~ 0
SPI_CS_B_N
Wire Wire Line
	4300 4300 4050 4300
Text HLabel 4300 4400 2    50   BiDi ~ 0
SPI_SDIO
Wire Wire Line
	4300 4400 4050 4400
Text HLabel 4300 4500 2    50   Input ~ 0
SPI_SCLK
Wire Wire Line
	4300 4500 4050 4500
Text Notes 2800 5850 0    50   ~ 0
SPI interface runs at 5V levels from U2 to MCU\nMCU to U2 runs at 3.3V\nADL5205 Vih is 2V so no level shift needed
Text HLabel 4300 4000 2    50   Input ~ 0
VGA_PERF_MODE
Wire Wire Line
	4300 4000 4050 4000
$Comp
L device:C C17
U 1 1 5E7A83A9
P 6200 3900
F 0 "C17" H 6315 3946 50  0000 L CNN
F 1 "4.7 uF 50V" H 6200 3800 50  0000 L CNN
F 2 "azonenberg_pcb:EIA_0805_CAP_NOSILK" H 6238 3750 50  0001 C CNN
F 3 "" H 6200 3900 50  0001 C CNN
	1    6200 3900
	1    0    0    -1  
$EndComp
Text Label 6200 3750 2    50   ~ 0
5V0_P
Text Label 6200 4050 2    50   ~ 0
GND
$Comp
L device:C C18
U 1 1 5E7A8E6C
P 6750 3900
F 0 "C18" H 6865 3946 50  0000 L CNN
F 1 "0.47 uF 25V" H 6750 3800 50  0000 L CNN
F 2 "azonenberg_pcb:EIA_0402_CAP_NOSILK" H 6788 3750 50  0001 C CNN
F 3 "" H 6750 3900 50  0001 C CNN
	1    6750 3900
	1    0    0    -1  
$EndComp
$Comp
L device:C C19
U 1 1 5E7A90CE
P 7300 3900
F 0 "C19" H 7415 3946 50  0000 L CNN
F 1 "0.47 uF 25V" H 7300 3800 50  0000 L CNN
F 2 "azonenberg_pcb:EIA_0402_CAP_NOSILK" H 7338 3750 50  0001 C CNN
F 3 "" H 7300 3900 50  0001 C CNN
	1    7300 3900
	1    0    0    -1  
$EndComp
Wire Wire Line
	7300 3750 6750 3750
Connection ~ 6750 3750
Wire Wire Line
	6750 3750 6200 3750
Wire Wire Line
	6200 4050 6750 4050
Connection ~ 6750 4050
Wire Wire Line
	6750 4050 7300 4050
$Comp
L Connector:Conn_01x01 TP?
U 1 1 5EAC6C0A
P 5550 1500
AR Path="/5E7383DD/5EAC6C0A" Ref="TP?"  Part="1" 
AR Path="/5E743C43/5EAC6C0A" Ref="TP10"  Part="1" 
F 0 "TP10" H 5630 1542 50  0000 L CNN
F 1 "TESTPOINT" H 5630 1451 50  0000 L CNN
F 2 "azonenberg_pcb:TESTPOINT_SMT_0.5MM" H 5550 1500 50  0001 C CNN
F 3 "~" H 5550 1500 50  0001 C CNN
	1    5550 1500
	1    0    0    -1  
$EndComp
$Comp
L Connector:Conn_01x01 TP?
U 1 1 5EAC8228
P 5550 1750
AR Path="/5E7383DD/5EAC8228" Ref="TP?"  Part="1" 
AR Path="/5E743C43/5EAC8228" Ref="TP11"  Part="1" 
F 0 "TP11" H 5630 1792 50  0000 L CNN
F 1 "TESTPOINT" H 5630 1701 50  0000 L CNN
F 2 "azonenberg_pcb:TESTPOINT_SMT_0.5MM" H 5550 1750 50  0001 C CNN
F 3 "~" H 5550 1750 50  0001 C CNN
	1    5550 1750
	1    0    0    -1  
$EndComp
Text Label 5350 1250 2    50   ~ 0
GND
Text Label 5350 1500 2    50   ~ 0
VGAIN_P
Text Label 5350 1750 2    50   ~ 0
VGAIN_N
$Comp
L Connector:Conn_01x01 TP?
U 1 1 5EAC8AE1
P 5550 1250
AR Path="/5E7383DD/5EAC8AE1" Ref="TP?"  Part="1" 
AR Path="/5E743C43/5EAC8AE1" Ref="TP9"  Part="1" 
F 0 "TP9" H 5630 1292 50  0000 L CNN
F 1 "TESTCLIP" H 5630 1201 50  0000 L CNN
F 2 "azonenberg_pcb:TESTPOINT_SMT_KEYSTONE_5016" H 5550 1250 50  0001 C CNN
F 3 "~" H 5550 1250 50  0001 C CNN
	1    5550 1250
	1    0    0    -1  
$EndComp
Wire Wire Line
	5050 2250 5050 2850
Wire Wire Line
	4750 2250 5050 2250
Text Notes 2800 1600 0    50   ~ 0
Swap P/N for routability
$EndSCHEMATC
